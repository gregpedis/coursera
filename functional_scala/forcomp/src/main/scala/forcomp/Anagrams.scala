...
object Anagrams extends AnagramsInterface {
  ...
  def wordAnagrams(word: Word): List[Word] = {
    ...
    dictionaryByOccurrences.getOrElse(key, List(word))
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    def recCombinations(
      ...
      rest match {
        ...
        case (ch, amount) :: tl => {
          val resultsWithCurrent =
            (
              for {
                i <- 1 to amount
                newPrefix = (ch, i) :: prefix
              } yield recCombinations(newPrefix, tl)
            ).flatten.toList

          val resultsWithoutCurrent = recCombinations(prefix, tl)
          resultsWithCurrent ++ resultsWithoutCurrent
        }
      }
    }

    recCombinations(Nil, occurrences).map(occ => occ.reverse)
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val xMap = x.toMap
    val res =
      y.foldLeft(xMap)((acc, kv) => acc.updated(kv._1, xMap(kv._1) - kv._2))
    res.filter(kv => kv._2 > 0).toList.sortBy(kv => kv._1)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def getMatchingWords(o: Occurrences) =
      dictionaryByOccurrences.getOrElse(o, Nil)
