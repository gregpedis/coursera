...
trait Huffman extends HuffmanInterface {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def same(x: Char)(y: Char): Boolean = x == y
    def howMany(x: Char) = chars.count(same(x))

    chars.toSet.map(c => (c, howMany(c))).toList
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    freqs.sortBy(kv => kv._2).map((c, w) => Leaf(c, w))
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    trees match {
      case l1 :: l2 :: rest => {
        (makeCodeTree(l1, l2) :: rest).sortBy(k => weight(k))
      }
      case _ => trees
    }
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    lazy val rootFork = tree match {
      case f: Fork => f
      case l: Leaf => throw KeyException("Not allowed bitch")
    }
    ...
    def getNewValues(
      ...
      currentTree match {
        case l: Leaf => (rootFork, l.char :: result)
        case f: Fork => (f, result)
      }
    }

    def recDecode(
        remaining: List[Bit]
    )(currentTree: Fork, result: List[Char]): List[Char] = {
      remaining match {
        case 0 :: xs =>
          recDecode(xs) _ tupled getNewValues(currentTree.left, result)
        case 1 :: xs =>
          recDecode(xs) _ tupled getNewValues(currentTree.right, result)
        case x :: xs => throw KeyException("What the fuck is this bit?")
        case _       => result
      }
    }

    tree match {
      case f: Fork => recDecode(bits)(f, Nil).reverse
      case l: Leaf => bits.map(b => l.char)
    }
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def getBits(c: Char): List[Bit] = {
      ...
      def recGetBits(currentTree: CodeTree, result: List[Bit]): List[Bit] = {
        currentTree match {
          case l: Leaf => result
          case f: Fork =>
            if contained(f.left) then recGetBits(f.left, 0 :: result)
            else recGetBits(f.right, 1 :: result)
        }
      }

      recGetBits(tree, Nil).reverse
    }

    text.map(getBits).flatten
  }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    val maybe = table.find((c, bits) => c == char)
    maybe match {
      case Some((c, bits)) => bits
      case None            => throw KeyException("What the fuck nintendo")
    }
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    ...
    tree match {
      case l: Leaf => List((l.char, getBits(l.char)))
      case f: Fork => f.chars.map(c => (c, getBits(c)))
    }
  }
